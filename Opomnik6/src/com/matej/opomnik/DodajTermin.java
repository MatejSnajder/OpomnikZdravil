                ...
                if(aliVsePravilno == 0)
                {
                	...
                	if(alipravilno)
                	{
	                	try
	                	{
		                	...
		                	Log.d("ww", "ee");
		                	//sedaj morem napolniti bazo med datumi.
		                	Log.d("mm", vnosnopoljeZacetekJemnjaString);
		                	Log.d("mm", vnosnopoljeKonecJemnjaString);
		                	
		                	
		                	int k1 = vnosnopoljeZacetekJemnjaString.indexOf('.');
		                	int k2 = vnosnopoljeZacetekJemnjaString.lastIndexOf('.');
		                	
		                	String dZ = vnosnopoljeZacetekJemnjaString.substring(0, k1);
		                	String mZ = vnosnopoljeZacetekJemnjaString.substring(k1+1, k2);
		                	String lZ = vnosnopoljeZacetekJemnjaString.substring(k2+1, vnosnopoljeZacetekJemnjaString.length());
		                	
		                	int m1 = vnosnopoljeKonecJemnjaString.indexOf('.');
		                	int m2 = vnosnopoljeKonecJemnjaString.lastIndexOf('.');
		                	
		                	String dK = vnosnopoljeKonecJemnjaString.substring(0, m1);
		                	String mK = vnosnopoljeKonecJemnjaString.substring(m1+1, m2);
		                	String lK = vnosnopoljeKonecJemnjaString.substring(m2+1, vnosnopoljeKonecJemnjaString.length());
		                	
		                	
		                	
		                	int letoZacetek = Integer.parseInt(lZ);
		                	int mesecZacetek = Integer.parseInt(mZ);
		                	int danZacetek = Integer.parseInt(dZ);
		                	Log.d("gg", "gg");
		                	int letoKonec = Integer.parseInt(lK);
		                	int mesecKonec = Integer.parseInt(mK);
		                	int danKonec = Integer.parseInt(dK);
		                	Log.d("zacetek", "z");
		                	if((mesecZacetek==mesecKonec)&&(letoZacetek==letoKonec))
		                	{
		                		for(int i=danZacetek; i<=danKonec; i++)
		                		{
		                			mySQLiteAdapter.openToWrite();
		                			
		                			mySQLiteAdapter.dodajNoviTermin("1", "1", vnosnopoljeNazivString,
				             	    		String.valueOf(i)+"."+String.valueOf(mesecZacetek)+"."+String.valueOf(letoZacetek), vnosnopoljeUraJemanjaString,
				             	    		vnosnopoljeKolicinaString, vnosnopoljeUraOpombaString);
		                			
		                			 mySQLiteAdapter.close();
		                		}
		                	}
		                	else if((mesecZacetek!=mesecKonec)&&(letoZacetek==letoKonec))
		                	{
		                		Log.d("hasjfhdsjfhdjf", "jkdfkdsfk");
		                		for(int i=mesecZacetek; i<=mesecKonec; i++)
		                		{
		                			if(i==mesecZacetek)
		                			{
			                			for(int j=danZacetek; j<=31; j++)
			                			{
			                				mySQLiteAdapter.openToWrite();
			                				
			                				mySQLiteAdapter.dodajNoviTermin("1", "1", vnosnopoljeNazivString,
						             	    		String.valueOf(j)+"."+String.valueOf(i)+"."+String.valueOf(letoZacetek), vnosnopoljeUraJemanjaString,
						             	    		vnosnopoljeKolicinaString, vnosnopoljeUraOpombaString);
			                				
			                				 mySQLiteAdapter.close();
			                			}
		                			}
		                			else if(i==mesecKonec)
		                			{
		                				for(int j=1; j<=danKonec; j++)
		                				{
		                					mySQLiteAdapter.openToWrite();
